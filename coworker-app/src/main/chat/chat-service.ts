...
export function buildOrchestratorSystemPrompt(
  ...
  lines.push(
    ...
    "- Default to a single best-fit coworker response.",
    `- Select up to ${Math.max(1, maxCoworkerResponses)} coworkers only when additional responses add distinct value.`,
    "- Generate responses sequentially in your chosen order.",
    "- For each response: call `generate_coworker_response`.",
    "- For the second and later response in a turn, call `judge_coworker_similarity` against prior emitted coworker responses before deciding to emit.",
    "- If similarity is high and material value is low, skip that coworker and continue.",
    "- If response is materially different, call `emit_coworker_message`.",
