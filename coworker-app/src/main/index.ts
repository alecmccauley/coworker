...
const getApiUrl = (): string => process.env.COWORKER_API_URL || 'http://localhost:3000'
...
const registerIpcHandlers = (): void => {
  ...
  ipcMain.handle('auth:verifyCode', async (_event, email: string, code: string) => {
  ...
  ipcMain.handle('auth:isAuthenticated', async () => {
    ...
    try {
      ...
      const { AuthenticationSdkError } = await import('@coworker/shared-services')
      if (error instanceof AuthenticationSdkError) {
        // Access token expired, try refresh
        try {
          const newTokens = await (await getSdk()).auth.refresh(tokens.refreshToken)
          authStorage.saveTokens(newTokens.accessToken, newTokens.refreshToken)
