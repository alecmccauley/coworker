...
import { openDatabase, closeDatabase, runMigrations, type WorkspaceDatabase } from '../database'
 ...
 */
export async function createWorkspace(name: string, folderPath: string): Promise<WorkspaceInfo> {
  ...
  writeFileSync(join(workspacePath, 'manifest.json'), JSON.stringify(manifest, null, 2), 'utf-8')
...
export async function openWorkspace(workspacePath: string): Promise<WorkspaceInfo> {
  ...
  if (!existsSync(manifestPath)) {
    throw new Error(`Invalid workspace: missing manifest.json at ${workspacePath}`)
  }

  const manifest = JSON.parse(readFileSync(manifestPath, 'utf-8')) as WorkspaceManifest

  // Open database
  const dbPath = join(workspacePath, 'workspace.db')
  if (!existsSync(dbPath)) {
    throw new Error(`Invalid workspace: missing workspace.db at ${workspacePath}`)
  }
  ...
  console.log('[Workspace] Opened workspace:', manifest.name, 'at', workspacePath)
...
export async function showCreateWorkspaceDialog(
  ...
  const result = await dialog.showSaveDialog(parentWindow ?? BrowserWindow.getFocusedWindow()!, {
    ...
    defaultPath: join(app.getPath('documents'), `My Workspace${WORKSPACE_EXTENSION}`),
...
export async function showOpenWorkspaceDialog(
  ...
  const properties: NonNullable<Electron.OpenDialogOptions['properties']> = ['openDirectory']
